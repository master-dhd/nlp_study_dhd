...
from evaluate_bert import BertEvaluator
...
def main(config):
    ...
    # 加载模型和tokenizer
    try:
        logger.info(f"加载BERT模型: {config['bert_model_path']}")
    except UnicodeEncodeError:
        logger.info(f"Loading BERT model: {config['bert_model_path']}")
    ...
    train_dataloader = load_bert_data(config["train_data_path"], config, logger, shuffle=True)
